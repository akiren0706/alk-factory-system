...
if tab_factories:
    ...
    for tab, fac in zip(tabs, tab_factories):
        with tab:
            ...
            tbl["表示名"] = tbl.apply(
                lambda r: r["indicator_jp"] if r["indicator_jp"] else r["indicator_ru"][:40], axis=1
            )
...
with st.expander("📋 全指標一覧"):
    ...
    show["指標"] = show.apply(
        lambda r: f"{r['indicator_jp']}  ({r['indicator_ru'][:30]})" if r["indicator_jp"]
                  else r["indicator_ru"][:50], axis=1
    )
    col_labels = {
