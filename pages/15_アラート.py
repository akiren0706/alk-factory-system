...
sel_factories = ALL_FACTORIES

# ── データ取得 ────────────────────────────────────────────────
today     = jst_today()
date_from = date(2025, 1, 1)   # 2025年1月1日から全件チェック

if st.button("🔄 データ更新", key="alert_refresh"):
    st.cache_data.clear()
    st.rerun()
...
for fac in sel_factories:
    ...
    if df_f.empty:
        ...
        _last_as_date = last_d if isinstance(last_d, date) else date.fromisoformat(str(last_d))
        delta   = (today - _last_as_date).days
