...
def _build_pdf_bytes(w_sun, w_sat, df_stop_w, df_op_w) -> bytes:
    ...
    try:
        ...
        sub_style   = ParagraphStyle("sub", fontName=font_name, fontSize=10,
                                     spaceAfter=4, textColor=colors.HexColor("#5C6B7A"))
        hdr_style   = ParagraphStyle("hdr", fontName=font_name, fontSize=11,
                                     spaceBefore=8, spaceAfter=4,
                                     textColor=colors.HexColor("#4E79A7"))

        story = []
        story.append(Paragraph("ALK 工場管理システム  週次レポート", title_style))
        story.append(Paragraph(
            f"{w_sun.strftime('%Y年%m月%d日')} 〜 {w_sat.strftime('%m月%d日')}",
            sub_style))
        story.append(Spacer(1, 6*mm))

        # 停止サマリー
        story.append(Paragraph("■ 停止データ サマリー", hdr_style))
        if df_stop_w.empty:
            story.append(Paragraph("停止データなし", sub_style))
        else:
            total_h = df_stop_w["duration_minutes"].sum() / 60
            story.append(Paragraph(
                f"停止件数: {len(df_stop_w)} 件 ／ 総停止時間: {total_h:.1f} 時間", sub_style))
            # 工場別テーブル
            fac_s = (df_stop_w.groupby("factory")
                     .agg(件数=("id","count"), 合計分=("duration_minutes","sum"))
                     .reset_index())
            fac_s["時間"] = (fac_s["合計分"] / 60).round(1)
            tdata = [["工場", "件数", "停止時間(h)"]]
            for _, r in fac_s.iterrows():
                tdata.append([r["factory"], str(int(r["件数"])), f"{r['時間']:.1f}"])
            t = Table(tdata, colWidths=[60*mm, 30*mm, 40*mm])
        ...
        story.append(Spacer(1, 6*mm))
        story.append(Paragraph("■ 生産指標 サマリー", hdr_style))
        if df_op_w.empty:
            story.append(Paragraph("生産指標データなし", sub_style))
        else:
            story.append(Paragraph(
                f"取込工場数: {df_op_w['factory'].nunique()} ／ 指標件数: {len(df_op_w)}", sub_style))
...
_pdf = _build_pdf_bytes(w_sun, w_sat,
                        get_stoppages("", str(w_sun), str(w_sat)),
                        get_operative("", str(w_sun), str(w_sat)))
if _pdf:
    st.download_button(
        "📄 PDF出力",
        data=_pdf,
        file_name=f"週報_{w_sun.strftime('%Y%m%d')}.pdf",
